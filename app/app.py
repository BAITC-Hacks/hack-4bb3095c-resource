...
with tab_excess:
    ...
    st.caption("Излишек = остаток + товары в пути − (спрос до следующей поставки + страховой запас). "
...
with tab_bt:
    ...
    if acc_path.exists():
        ...
        st.caption("Отсечки 01.04, 01.05, 01.06.2026 → прогноз на 3 месяца вперёд; только месяцы, когда товар был в "
                   "наличии. Ошибка прогноза — насколько в сумме прогноз промахнулся, в % от спроса (меньше — лучше). База — «как в Excel»: "
                   "среднее за 12 месяцев.")
        st.dataframe(pd.DataFrame([{"Поставщик": k, "Ошибка прогноза сервиса": f"{v['wape_ours']:.1%}",
                                    "Ошибка «среднее за 12 мес.»": f"{v['wape_naive_12m_avg']:.1%}",
