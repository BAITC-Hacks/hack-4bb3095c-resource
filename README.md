...
| Масштабирование на любую компанию | «Данные и параметры» → «Загрузить свой файл»: один Excel с листами `sales`, `stock_now`, `transit`, `items`, `stock_hist`; кнопка «Скачать шаблон (пример)» | `tests/test_upload.py` |
...
## Ограничения
- «Материальная ведомость из 1С» (упомянута во входах ТЗ) в выданных партнёром данных отсутствует; явные периоды
  stockout можно передать листом `stockouts` (sku, month, avail) при загрузке своих данных — иначе они определяются
  по помесячным остаткам.
